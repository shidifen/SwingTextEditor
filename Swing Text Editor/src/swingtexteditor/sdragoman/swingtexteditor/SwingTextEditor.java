...
public class SwingTextEditor extends JFrame {
	...
	//when no other doc open, this will be used to display the text
	//if the nr of docs > 1, a JTabbedPane will be used
	...
	public SwingTextEditor() {
		...
		//this was 0 initially, the default Java value for int
		++nrOfOpenDocs;
	...
	//widely used function; it provides the correct textArea,
	//as there are two cases: when there's 1 doc open, it returns
	//the textArea associated with the enclosing class; else, 
	//it gets the correct textArea from one of the tabs
	private CustomJTextArea getCustomJTextArea(){
		//at a minimum, there are 3 tabs open: first 2 are docs, 
		//the third one is the "add tab" button with a "+"
		if (tabs.getTabCount() < 3) {
			return textArea;
		}
		//only one doc open, no use for tabs
		else {
			//if the user clicks next to the "add tab" button, the tab
			//above will be selected, we must avoid an error
	...
	//the text area to be used for a single doc or all the tabs
	...
	//this is what actually builds the editor; huge class
	private class LotsOfMenusBuilder {
		...
		//for diversity only *this is a test program after all*
		{
			...
			//classes needed for undo-redo actions
			...
			//little plus sign for adding a new tab
			...
			//used for edit-copy-paste actions
		...
		void addEventHandlersToMenuItems() {
			...
			//this will detect when different tabs are selected
			//useful for seting the titles of the tab/program
			tabs.addChangeListener(new ChangeListener() {
				public void stateChanged(ChangeEvent e) {
					//this index will be selected after a tab is closed, i.e. no one
			...
			//this contains a tic-tac-toe game :)
		...
		//create a textArea regardless of the nr of docs open
